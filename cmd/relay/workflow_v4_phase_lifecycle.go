...
const (
	workflowV4ProtocolLabsDrand = "https://api.drand.sh"
	workflowV4CloudflareDrand   = "https://drand.cloudflare.com"
)
...
func fetchWorkflowV4QuicknetRoundFrom(origin string, round uint64, destination string) error {
	...
	if origin != workflowV4ProtocolLabsDrand && origin != workflowV4CloudflareDrand {
		return errors.New("unsupported drand relay origin")
	}
	...
	url := fmt.Sprintf("%s/%s/public/%d", origin, workflowV4QuicknetChainHash, round)
