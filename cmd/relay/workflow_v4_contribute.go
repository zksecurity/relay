...
func (j *workflowV4Journal) executePreparedContribution(id, dockerCLI string, snapshot storagefirst.SnapshotV4, protocol transcript.DefinitionProtocol, local storagefirst.LocalTurnV4) error {
	...
	d := &dockerDriver{image: p.Runtime.Image, platform: p.Runtime.Platform, ceremonyBinary: "/usr/local/bin/mpc-ceremony", root: o.root, definition: o.definition, definitionSig: o.definitionSig, coordinatorKey: o.coordinatorKey, signingKey: o.signingKey, environment: o.envPath, candidateRoot: filepath.Dir(o.outDir), executionIntentPath: workflowV4ContributorIntentPath(b.Work, p.ID), client: osDockerCommandClient{binary: dockerCLI}, now: time.Now}
	o.docker = d
