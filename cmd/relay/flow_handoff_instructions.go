...
func handoffInstructionFor(id string) (handoffInstruction, bool) {
	all := map[string]handoffInstruction{
		...
		"decision-scope":              {"Your coordinator and accountable roles", "No new file transfer is requested. Retain the verified production decision and required signatures, or the authenticated no-GO-applicability result for rehearsal mode. GO approves only the circuit named by the signed definition.", nil},
