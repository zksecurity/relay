...
func assurance(task flowTask, control string) flowTask { task.Assurance = control; return task }
...
func submitFlow(role, phase string) flowTask {
	...
	return flowTask{ID: "submit", Label: "Upload signed PUBLIC output", Help: "Stage only the signed public output, never signing keys or access-file contents. A Tessera connection reports the manifest automatically; submission is still not acceptance.", Command: command, Fields: []flowField{profileField(role, phase), ff("grant", "Private Tessera connection or fresh role grant", "/work/"+role+".grant.json"), ff("dir", "Directory containing only signed public outputs", "/work/signed-output")}}
}
...
func decisionFlow(role string) flowStage {
	stage := flowStage{ID: "decision", Label: "Production decision (not applicable to rehearsal mode)"}
	if role == "coordinator" {
		prepare := withFields(flowProof("prepare-decision", "Prepare the canonical GO/NO-GO decision", "Production mode only. Supply the reviewed decision draft and evidence for the exact signed circuit and release. Rehearsal mode is ineligible.", "decision", "prepare"), ff("draft", "Reviewed production decision draft", "/work/decision-draft.json"), ff("out", "Fresh canonical decision", "/work/decision.json"))
	...
	stage.Tasks = append(stage.Tasks, handoff("decision-scope", "Record decision scope and authorization", "For production mode, retain the verified decision and every required accountable signature. GO approves only the exact signed circuit and release; a test circuit cannot produce ownership proofs. For rehearsal mode, record that this gate is not applicable."))
	return stage
}

func coordinatorFlowStages() []flowStage {
	stages := []flowStage{{ID: "enrollments", Label: "Assignments and enrollments", Tasks: []flowTask{
		inspectFlow(), handoff("assignments", "Share the signed ceremony files with each role", "Send the listed public files through your agreed coordination channel so each role can review the ceremony and create its enrollment. Recipients must independently authenticate the coordinator key. Ask them to return their public enrollment folder: canonical.json, enrollment.sig, and the referenced disclosure. Never send signing.hex, credentials or private grants. This records sharing only, not recipient review or enrollment."), enrollmentFlow(),
	...
	for _, phase := range []string{"phase1", "phase2"} {
		...
		stages = append(stages, flowStage{ID: phase + "-turns", Label: phase + " participant turns", Tasks: []flowTask{inspectFlow(), custodyPrepareFlow(phase, "outbound"), custodySignFlow(phase, "outbound", "handoff"), handoff("deliver-input", "Deliver the signed input packet and collect its signed receipt", "Send the exact handoff, signature and named public payload to the assigned participant. Wait for their signed receipt before authorizing computation."), custodyVerifyFlow(phase, "outbound"), grant, custodyReceiptFlow(phase, "return"), custodySignFlow(phase, "return", "receipt"), accept, handoff("turns-complete", "Confirm all intended turns are accepted", "Repeat grant and acceptance for each scheduled participant. Reauthenticate the head after each acceptance. Closing will independently enforce the signed minimum; do not treat an inbox listing as acceptance.")}})
		...
		publishBeacon := publishFlow(phase, true)
		publishBeacon.ID = "publish-beacon"
		publishBeacon.Label = "Publish the recorded beacon for mirrors and auditors"
		witnessesReady := assurance(handoff("witnesses-ready", "Confirm witnesses are observing", "Ask each required witness to confirm readiness through your coordination channel."), "witness")
		observations := assurance(handoff("observations", "Collect the public-witness evidence", "Witnesses must preserve exact public closure bytes and sign only what they observed before the beacon window ends. Missing or conflicting evidence requires investigation."), "witness")
		stages = append(stages, flowStage{ID: phase + "-close", Label: phase + " closure and public beacon", Tasks: []flowTask{witnessesReady, closeTask, publishFlow(phase, true), observations, beacon, publishBeacon}})
		if phase == "phase1" {
			...
			republish := publishFlow("phase1", true)
			republish.ID = "publish-phase1-seal"
			republish.Label = "Publish the Phase 1 beacon, seal and commons"
			stages = append(stages, flowStage{ID: "phase2-init", Label: "Prepare Phase 2", Tasks: []flowTask{seal, republish, init, publishFlow("phase2", false)}})
		}
	}
	...
	tinyProof := flowTask{ID: "tiny-public-proof", Label: "Generate public proof evidence for a ceremony test circuit", Help: "For rehearsal-tiny-v1 or rehearsal-k11-v1. This authenticates preliminary keys and proves the fixed public test statement. The result is not an ownership proof.", Command: []string{"mpc-ceremony", "finalize", "rehearsal-evidence"}, Fields: []flowField{ff("keys-dir", "Authenticated preliminary keys", "/work/preliminary"), ff("coordinator-public-key-file", "Trusted coordinator key", "/trust/setup-coordinator.hex"), ft("ceremony-id", "Exact signed ceremony ID", ""), ff("out", "Fresh public proof evidence", "/work/public-finalization-evidence.json")}, Optional: true}
	audits := assurance(handoff("audits", "Send the exact candidate for independent audits", "Each required enrolled auditor independently acquires and replays the complete transcript. Collect signed passing reports and resolve incidents before requesting final signing."), "audit")
	stages = append(stages, flowStage{ID: "finalization", Label: "Finalize and prepare review", Tasks: []flowTask{prepare, tinyProof, handoff("public-proof", "Obtain public finalization evidence", "Give the preliminary keys to the circuit's public-evidence generation process. It returns the public proof artifact. This guide does not generate private application inputs."), complete, audits}})
	...
	signOps.Fields = append(signOps.Fields, ft("reviewed-sha256", "SHA-256 of the exact bundle you reviewed", ""))
	signOps.Offline = true
	stages = append(stages, flowStage{ID: "operational-evidence", Label: "Collect, prepare and sign the evidence bundle", Tasks: []flowTask{prepareOps, signOps}})
	ops := withFields(flowProof("ops-verify", "Verify the complete signed operational bundle", "Verifies the coordinator's bundle signature and every referenced record. Finding a file or reporting a handoff does not satisfy this gate. Authenticated claims do not prove physical independence or erasure.", "ops", "verify"), ft("record-type", "Operational record type", "evidence-bundle"), ff("record", "Signed evidence bundle", "/work/ceremony/public/operational/evidence-bundle.json"), ff("signature", "Bundle signature", "/work/ceremony/public/operational/evidence-bundle.sig"), ff("signer-public-key-file", "Trusted bundle signer key", "/trust/coordinator-public-key.hex"), ff("evidence-root", "Complete evidence root", "/work/ceremony/public"))
...
func baseRoleFlowStages(role string) []flowStage {
	...
	if role == "participant" {
		stages := []flowStage{{ID: "assignment", Label: "Identity and assignment review", Tasks: []flowTask{handoff("assignment", "Review your identity, assignment and cleanup precautions", "Generate your own identity and send only identity.json to the coordinator. Receive authenticated phase profiles. Confirm your assigned positions and independently authenticate the coordinator key. Docker cleanup does not exclude host/VM remnants. On Linux, use native Docker Engine with host swap disabled; Macs use Docker Desktop.")}}}
		for _, phase := range []string{"phase1", "phase2"} {
			...
			run := flowTask{ID: "contribute", Label: "Contribute, confirm cleanup and upload", Help: "Wait for your coordinator's turn notice. A Tessera connection obtains fresh scoped upload access; a standalone ceremony uses a fresh private grant. The host supervisor runs the isolated contributor, verifies removal, then separately asks about retained copies before signing/uploading.", Command: []string{"run"}, Fields: []flowField{config, {Flag: "grant", Label: "Private Tessera connection or fresh role grant", Kind: "host"}, {Flag: "resume-candidate", Label: "Existing public candidate after an interrupted turn", Kind: "host", Optional: true}}}
			stages = append(stages, flowStage{ID: phase, Label: phase + " contribution", Tasks: []flowTask{status, custodyReceiptFlow(phase, "outbound"), custodySignFlow(phase, "outbound", "receipt"), handoff("deliver-input-receipt", "Return your signed receipt before computing", "Deliver your exact signed input receipt to the coordinator. Wait for their authorization and grant before starting."), run, custodyPrepareFlow(phase, "return"), custodySignFlow(phase, "return", "handoff"), handoff("deliver-output", "Deliver the public candidate and signed return packet", "Send the exact public candidate files and signed handoff to the coordinator. They must sign the receipt before accepting this candidate."), handoff("accepted", "Confirm independently verified coordinator acceptance", "A Tessera connection reports the manifest automatically; otherwise send it to the coordinator. A successful upload is not acceptance. After an interruption preserve the candidate, get fresh access, and resume rather than recompute.")}})
		}
	...
	case "release-signer":
		...
		stages = append(stages, flowStage{ID: "sign", Label: "Review, sign and hand off", Tasks: []flowTask{sign, releaseVerifyFlow(), handoff("handoff", "Transfer only the signed public release", "Give the signed public release to the online recipient shown by your pinned ceremony guide. Keep your private key offline; retain the verification and transfer records. Complete any required production decision before release authorization.")}})
	case "upload-station":
		stages = append(stages, flowStage{ID: "upload", Label: "Public release handoff", Tasks: []flowTask{releaseVerifyFlow(), submitFlow("release", "phase2"), handoff("accepted", "Confirm independently verified acceptance", "A Tessera connection reports the manifest automatically; otherwise send it to the coordinator. This station must not hold signing keys.")}})
	default:
		return nil
	}
