...
func coordinatorFlowStages() []flowStage {
	...
	prepareOps.Fields = append(prepareOps.Fields, flowField{Flag: "witness-quorum", Label: "Agreed minimum witnesses per phase (1-32; do not lower it to fit available receipts)", Default: "1", Kind: "number"})
...
func roleFlowStages(role string) []flowStage {
	...
	case "release-signer":
		...
		sign := withFields(flowProof("sign", "Verify audits/evidence and sign the final parameters", "Requires at least one enrolled auditor and the signed operational bundle for both phases. This never mutates the candidate.", "release", "sign"), ff("candidate-bundle", "Exact audited candidate", "/work/candidate"))
