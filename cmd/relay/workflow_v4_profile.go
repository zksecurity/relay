...
func workflowV4ProfileBinding(p, signer guidedProfile, protocol transcript.DefinitionProtocol, identity setupIdentity, participant *access.RoleConfig) (workflowV4Binding, error) {
	...
	if !protocol.UsesV4() || (p.Role != "coordinator" && p.Role != "participant") || len(p.Command) != 0 {
		return zero, errors.New("storage-first turn guide requires an authenticated V4 coordinator or participant profile")
	}
