...
func runCoordinator(args []string) error {
	if len(args) == 0 {
		return errors.New("coordinator requires prepare, configure-storage, grant, candidates, accept, evidence, publish, fetch-candidate-v4, fetch-enrollment-v4, or commit-v4")
	}
	switch args[0] {
	case "prepare-local":
