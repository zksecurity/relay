...
func TestParticipantRunLockIsExclusivePerProfile(t *testing.T) {
	...
	other, err := acquireParticipantRunLock(filepath.Join(t.TempDir(), "other.json"), candidates)
	if err != nil {
		t.Fatalf("different profile was blocked: %v", err)
	}
