...
export const beacon = (mode, selectedLead = mode === "production" ? 86400 : 180) => ({ ...beaconProfile, minimum_witness_lead_seconds: selectedLead });
...
export const inputDigest = ({ schema, id, plan_revision, plan }) => digest("ceremony-setup-input-v3", { schema, id, plan_revision, plan });
export const resultDigest = result => digest("ceremony-setup-result-v3", result);
export const setupDigest = setup => digest("ceremony-setup-v3", setup);
...
export function parseSetup(bytes) {
  ...
  const p = setup.plan;
  require(canonical(p.beacon_policy) === canonical(beacon(p.mode, p.beacon_policy.minimum_witness_lead_seconds)), "Beacon policy must match the approved profile except for its selected lead time");
  ...
  require(counts.get("coordinator") === 1 && counts.get("release-signer") === 1, "Setup requires one coordinator and one release signer");
  const assurance = p.assurance_policy;
  require(assurance.passing_ceremony_audits <= (counts.get("auditor") || 0) && (assurance.passing_ceremony_audits !== 0 || !counts.get("auditor")), "Auditor assignments must match the signed ceremony-audit requirement");
  // Witness and mirror identities are assigned and enrolled after initialization;
  // only their required counts are part of the authenticated setup.
  require(assurance.external_security_audit_signoffs === 0, "External security audit signoffs are not supported by this setup contract");
