...
The CLI retains the imported account/buckets and asks for three protected files:
the AWS-format coordinator profile, inbox-only secret (`parent`), and privacy-check
token (`control`). It saves its own copies after confirmation, then recommends
Check storage. Source files remain untouched, including for session-only copies;
file validation does not prove cloud access or permissions.
...
- Keep the coordinator and inbox-parent R2 credentials separate. A compromised
  ...
  The coordinator credential could technically serve both purposes, but Relay
  requires separation to limit the damage from a leaked credential or a scoping
  mistake. The CLI calls the second one the **inbox-only credential**: give it
  **Object Read & Write** access to the selected inbox bucket only.
